...
__global__ void raytrace_kernel(uint32_t n_elements, const Vector3f* __restrict__ rays_o, const Vector3f* __restrict__ rays_d, Vector3f* __restrict__ positions, Vector3f* __restrict__ normals, float* __restrict__ depth, int32_t * face_id, const TriangleBvhNode* __restrict__ nodes, const Triangle* __restrict__ triangles);
...
public:
    ...
    void ray_trace_gpu(uint32_t n_elements, const float* rays_o, const float* rays_d, float* positions, float* normals, float* depth, int32_t* face_id, const Triangle* gpu_triangles, cudaStream_t stream) override {
...
__global__ void raytrace_kernel(uint32_t n_elements, const Vector3f* __restrict__ rays_o, const Vector3f* __restrict__ rays_d, Vector3f* __restrict__ positions, Vector3f* __restrict__ normals, float* __restrict__ depth, int32_t* __restrict__ face_id, const TriangleBvhNode* __restrict__ nodes, const Triangle* __restrict__ triangles) {
    uint32_t i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= n_elements) return;
